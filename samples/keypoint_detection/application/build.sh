...
clean() {
    ...
    docker run --rm \
        -v $PWD/:$PWD/ \
        $NAME_IMAGE \
        /bin/sh -c "cd ${PWD}/${KEYPOINT_DETECTION_SAMPLE}/sample/vision_app/single_dnn/keypointdetection && make clean --file=Makefile_VnS"
}
...
build_keypointdetection() {
    echo "build_keypointdetection"
    builddockerimage
    docker run --rm \
        -v $PWD/:$PWD/ \
        $NAME_IMAGE \
        /bin/sh -c "cd ${PWD}/${KEYPOINT_DETECTION_SAMPLE}/sample/vision_app/single_dnn/keypointdetection && make ${BUILD_TYPE} --file=Makefile_VnS"
}
