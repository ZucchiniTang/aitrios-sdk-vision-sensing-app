...
KEYPOINT_DETECTION_SAMPLE='.devcontainer/dependencies/aitrios-sdk-keypoint-detection-webapp-cs/sample'
...
if [ "$USER_WASM_FILE" = "" ]; then
    if [ -n "$DEBUGGER" ] && [ $DEBUGGER = "-d" ]; then
        WASM_FILE=${PWD}/${KEYPOINT_DETECTION_SAMPLE}/sample/vision_app/single_dnn/keypointdetection/build/debug/vision_app_keypointdetection.wasm
    else
        WASM_FILE=${PWD}/${KEYPOINT_DETECTION_SAMPLE}/sample/vision_app/single_dnn/keypointdetection/build/release/vision_app_keypointdetection.wasm
    fi
else
...
# Build Wasm
$PWD/samples/keypoint_detection/application/build.sh -t $LOAD_PGM $DEBUGGER 
