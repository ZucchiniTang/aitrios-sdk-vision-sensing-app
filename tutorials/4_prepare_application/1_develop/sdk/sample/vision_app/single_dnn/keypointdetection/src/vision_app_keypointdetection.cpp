...
int main(int argc, char *argv[]) {
    ...
    while (!s_is_evp_exit) {
        ...
        ret = PPL_KeypointDetectionSsdAnalyze((float *)raw_data.address, raw_data.size/4, &p_out_buf, &out_size, s_ssd_param);
        INFO_PRINTF("Analyze done");

        if ((EPPL_RESULT_CODE)ret != E_PPL_OK) {
            ERR_PRINTF("PPL_KeypointDetectionSsdAnalyze : ret=%d", ret);
        }
        else {
